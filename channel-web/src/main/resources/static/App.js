...
import logo from './rescource/image/login/logo-gao.png'
import left from './rescource/image/login/login-left.svg'
...
class NormalLoginForm extends React.Component {
  ...
  render() {
    ...
    return (
        <Form onSubmit={this.handleSubmit} className={styles.loginForm} style={{margin:'300px auto',marginBottom:'0px'}}>
          <div className={styles.product_title}>体智能</div>
          {this.state.visible?(
          <Alert
              message={this.state.message}
              type="error"
              style={{background:'#FFEFF0',height:'30px',fontSize:'12px',color:'#F56C6C',border:'none',marginBottom:'10px',lineHeight:'15px'}}
              />):null}
          <Form.Item>
            {getFieldDecorator('userName', {
              rules: [{ required: true, message: '请输入您的域账号!' }],
            })(
                <Input onFocus={this.inputUserNameOnFocus} onBlur={this.inputUserNameOnBlur} prefix={<Icon type="user" style={{ color:this.state.userNameBg}} />} placeholder="域账号" name="userName"/>
            )}
          </Form.Item>
          <Form.Item>
            {getFieldDecorator('password', {
              rules: [{ required: true, message: '请输入您的密码!' }],
                ...
                <Input onFocus={this.inputPasswordOnFocus} onBlur={this.inputPasswordOnBlur} prefix={<Icon type="lock" style={{ color:this.state.passwordBg }} />} type="password" placeholder="请输入您的密码"  name="password"/>
            )}
          </Form.Item>
